# GCP 上的机器学习：从 Notebook 到数据流水线

> 原文：[`towardsdatascience.com/machine-learning-on-gcp-from-dev-to-prod-with-vertex-ai-c9e42c4b366f?source=collection_archive---------4-----------------------#2024-05-11`](https://towardsdatascience.com/machine-learning-on-gcp-from-dev-to-prod-with-vertex-ai-c9e42c4b366f?source=collection_archive---------4-----------------------#2024-05-11)

## Notebook 不足以支撑大规模的机器学习
...
·发表于 [Towards Data Science](https://towardsdatascience.com/?source=post_page---byline--c9e42c4b366f--------------------------------) ·15 分钟阅读·2024 年 5 月 11 日

--

![](img/a0f9612d168ba15a6cbadea7628ba7db.png)
...
![](img/880c9b38a51bc63c48840f0c89577208.png)
...
![](img/0b0974517e0f107036fa37d8339f6f23.png)![](img/5cb46a927920c0bb0df6c3465aa5ec81.png)
...
![](img/696d3dbba8791000cd8405119c1144d3.png)
...
1.  **成本**：运行一个笔记本意味着你需要一台能够运行它的机器。— 这台机器是有成本的，对于大型模型或庞大的数据集，你需要具有高规格的虚拟机。
    ...
    — Vertex AI 管道是一个*无服务器*服务，意味着你不需要管理底层基础设施，只需为你使用的部分付费，即执行时间。
...
至少使用 Docker 镜像来进行定时任务，或者如果你想做得更规范，可以使用管道，但永远不要在生产环境中运行笔记本。
...
第一个组件将从 Bigquery 下载数据并将其保存为 CSV 文件。

我们使用的 BASE_IMAGE 是我们之前构建的镜像！我们可以用它来导入我们在 Docker 镜像`src`文件夹中定义的模块和函数：
...
最后一步是计算指标（这些指标实际上是在模型训练过程中计算的）。这只是一个必要步骤，但它能让你看到构建轻量级组件是多么简单。注意，在这种情况下我们并没有从 BASE_IMAGE 构建组件（它有时可能非常大），而是仅仅构建了一个包含必要组件的轻量级镜像：
...
通常还会有其他步骤可以加入，例如如果我们希望将模型部署为 API 端点，但这属于更高级的内容，需要为模型的服务创建另一个 Docker 镜像。下一次会介绍。
...
如果一切顺利，你现在应该能在 Vertex UI 中看到你的管道：

![](img/ed0f2a97ea1def0df9a9ccdac40f0be6.png)

你可以点击它，查看不同的步骤：

![](img/c8ce67a3cf2ebdcff0665e1cbc227d01.png)
