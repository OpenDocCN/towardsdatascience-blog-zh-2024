...
手动计算十六进制颜色代码 ffab23 的 RGB 值。图示由作者提供。
...
在 Excel VBA 中，使用`**&H**`与一个值组合意味着它是一个十六进制数，而`Val()`函数返回相应的十进制数。在下面的立即窗口中，`r`、`g`和`b`分别代表红色、绿色和蓝色的相应十进制值。
...
当我运行上述子程序时，F 列会被涂上与颜色代码相同的颜色，如下所示：
...
在 Excel VBA 中使用字典的前提条件。图示由作者提供。
