...
数学规划系统（MPS）是一种用于表示线性和混合整数线性规划问题的[文件格式](https://en.wikipedia.org/wiki/MPS_(format))。它是一种相对较老的格式，但所有商业线性程序求解器都接受这种格式。线性问题也可以用其他格式表示，如 LP、AMPL 和 GAMS。
