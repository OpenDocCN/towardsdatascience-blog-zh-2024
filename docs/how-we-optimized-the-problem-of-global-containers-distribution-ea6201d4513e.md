...
> 原文：[`towardsdatascience.com/how-we-optimized-the-problem-of-global-containers-distribution-ea6201d4513e?source=collection_archive---------5-----------------------#2024-08-01`](https://towardsdatascience.com/how-we-optimized-the-problem-of-global-containers-distribution-ea6201d4513e?source=collection_archive---------5-----------------------#2024-08-01)
...
·发表于[Towards Data Science](https://towardsdatascience.com/?source=post_page---byline--ea6201d4513e--------------------------------) ·阅读时间 18 分钟·2024 年 8 月 1 日
...
1.  用来保证目标函数和约束条件是凸的技术——cvxpy 的主要限制。

事不宜迟，让我们开始吧。

# 1. 挑战

当项目启动时，公司向我们透露，他们已经在*Microsoft Excel Solver*上实现了解决方案，以优化如何最好地管理集装箱。该 Solver 旨在减少运输、货运、存储和操作的成本，同时遵循一系列约束条件。
...
![](img/001c29d34f438addc891bce6c9dbdac9.png)
...
![](img/fdbfee15861f3e4967091c415c6c6177.png)
...
![](img/d4b2327f58c1a5ca015aaba46d9191ed.png)
...
![](img/70d4a1a86c13c0f4e06e37a43132f0a3.png)
...
我们如何在 cvxpy 中实现这个呢？其实很简单。首先我们需要一个变量**x**，表示优化器可以做出的选择。在这种情况下，表示**x**的最佳方式是使用形状为(4, 2)的**布尔**数组——每行对应一个集装箱，2 列表示有两个发货人可供选择：

![](img/4684be83ca2133aa3fc0f31123427229.png)
...
+   代码行`constraint0 = cx.sum(x_shippers, axis=1) <= 1`是一个*cvxpy*必须遵守的约束条件，用于优化**x**。作为一般规则，约束条件必须保持优化过程的凸性（这保证了收敛性），并且可以是等式表达式或上界等式。在此案例中，`sum`运算符作用于*axis=1*，意味着“按列求和”。
    ...
    该规则意味着每行**x_shippers**的和最多等于 1，这保证了某个集装箱不会同时分配给多个发货人。

    由于求和约束遵循`<=`规则，因此某行可以全是 0，这意味着某个集装箱可能完全没有被分配给任何船只（例如，可能由于船只上没有空余空间）。
...
![](img/495c23a75b2347bb0018533dd00b4ab7.png)
...
![](img/0f2cdb29fbc489646b636f49b2111817.png)
...
![](img/21da9318f47e715e0fee80b97c4aff47.png)
...
![](img/b69fc16b735bed79ebcd1dddbc187c46.png)
...
![](img/8874df58b7448fd7c66dc8a8cc12d71e.png)
...
![](img/df1fd6f7697ef3aa218d18c7eb98f17d.png)
...
![](img/daa3a7baecd3d0d7526eeac54c141b83.png)
...
![](img/90a883b77ed8cd7d84dab80e3c7e522e.png)
...
![](img/3f8b642078389e35f315f02a01d75a48.png)

变量容器，第一列代表工厂，第二列代表目的地，第三列代表容器的数量。图像来源：作者。

整体思路其实是完全相同的。需要考虑的要点：

+   在第 72 行，代码`shipper_shipper_trade_arr = x_shippers @ shipper_shipper_and_trade_map.matrix`将发货人的选择转换到发货人和贸易的领域。通过这样做，我们可以汇总每个贸易和发货人分配的集装箱总数。

+   `constr23` 旨在强制过度预订，前提是优化器已经消耗了所有货运商的空间。通过将 `x_ob_shippers`（"ob" 表示过度预订）转化为货运商和交易领域来实现：

![](img/198432af76932f202378d54f6ad9b9f5.png)
