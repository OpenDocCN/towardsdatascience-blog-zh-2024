...
现在让我们来训练我们的模型。我们将从直接的提升模型——提升随机森林分类器开始。然后，我们将使用XGBoost回归器训练我们的元模型。这里有两点需要注意：
...
+   **Y轴（增益）**：这显示的是累积增益，即你在结果中的改进（例如，提高了留存率）。
...
1.  最后，我们注意到，超过40%的目标人群后，所有曲线开始下降，表明联系这些客户没有价值。
